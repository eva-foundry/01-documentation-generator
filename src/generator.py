...
# Import validators and evidence (local modules - updated for package structure)
try:
    from .validators import validate_yaml_frontmatter, validate_consumability, validate_traceability, validate_yaml_values, validate_required_sections
...
# Setup logging
log_file = Path(__file__).parent.parent.parent / "eva-foundation-generation.log"
logging.basicConfig(
    level=logging.INFO,
...
class EVAFoundationGenerator:
    ...
    def __init__(self, workspace_root: Path, dry_run: bool = False, skip_validation: bool = False):
        ...
        self.project_dir = self.eva_dir / "projects" / "01-documentation-generator"
    ...
    def _setup_api_logging(self):
        """Configure detailed logging for API calls"""
        # Create logs directory
        logs_dir = self.eva_dir / "generated-output" / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        
        # Configure file handler for API logs
        log_file = logs_dir / f"api-calls-{self.start_time.strftime('%Y%m%d-%H%M%S')}.log"
        ...
        console.print(f"[dim]   📝 API logging enabled: {log_file.name}[/dim]")
    
    def _load_system_prompt(self) -> str:
        """Load README.md system prompt with format exemplar"""
        ...
        # Load exemplar file to show exact format
        exemplar_path = self.project_dir / "prompts" / "example-architecture-doc.md"
        if exemplar_path.exists():
            with open(exemplar_path, 'r', encoding='utf-8') as f:
                exemplar = f.read()
            system_prompt += f"\n\n---\n\n## FORMAT EXEMPLAR\n\nHere is an example showing the EXACT format to follow:\n\n{exemplar}"
        
        return system_prompt
    
    def _load_exemplar(self) -> str:
        """Load exemplar file for inline injection in generation message"""
        exemplar_path = self.project_dir / "prompts" / "example-architecture-doc.md"
        if exemplar_path.exists():
            with open(exemplar_path, 'r', encoding='utf-8') as f:
                return f.read()
        return "(Exemplar not found - use YAML standards from system prompt)"
    ...
    def _init_llm(self) -> AzureChatOpenAI:
        ...
        try:
            ...
            return AzureChatOpenAI(
                ...
                api_key=api_key,
                deployment_name=os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME", os.getenv("AZURE_OPENAI_CHAT_DEPLOYMENT", "gpt-4.1-mini")),
    ...
    def generate_file(self, spec: FileSpec) -> tuple[str, ValidationResult]:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if self.dry_run:
                ...
                try:
                    # === OPTIMIZED HANDSHAKE PATTERN (2 calls instead of 4) ===
                    # Step 1: Setup (Role + Context together)
                    console.print(f"   [dim]Step 1: Establishing context...[/dim]")
                    setup_msg = [
                        SystemMessage(content=self.system_prompt),
                        HumanMessage(content=f"""Source files for reference:

{self._build_v02_context()}

Confirm you understand your role and have the source context by responding with 'Ready'.""")  
                    ]
                    
                    # API Call 1: Setup with logging
                    api_start = time.time()
                    logger.info(f"API Call #{self.api_call_count + 1} - Setup context for {spec.filename}")
                    setup_resp = self.llm.invoke(setup_msg)
                    api_duration = time.time() - api_start
                    self.api_call_count += 1
                    self.total_api_time += api_duration
                    logger.info(f"API Call #{self.api_call_count} completed in {api_duration:.2f}s")
                    
                    console.print(f"   [dim]  ✓ Context established[/dim]")
                    
                    # Step 2: Generate Request with exemplar INLINE for maximum salience
                    console.print(f"   [dim]Step 2: Requesting document...[/dim]")
                    gen_msg = setup_msg + [
                        setup_resp,
                        HumanMessage(content=f"""You MUST follow this EXACT format:
...
1. YAML keys in EXACT order: document_type, audience (multi-line), phase, traceability
2. Sections MUST be numbered: ## 1. In Scope, ## 2. Out of Scope, etc.
3. Audience MUST be multi-line list format:
   audience:
     - Role A
     - Role B
   (NEVER use inline: [A, B, C])
4. Traceability MUST follow format: source-materials/requirements-v0.2/FILE.md
5. Tables MUST match exemplar column structure exactly
6. Implementation Evidence section REQUIRED for phase 1+
7. Use requirement IDs from v0.2 sources only (INF01, ACC03, etc.)
8. Code blocks MUST have language tags (```python, ```bash)
9. File references MUST be explicit paths (app/backend/app.py)
10. document_type MUST be one of: [architecture, technical_design, operational, governance, conops]

---

Now generate: {spec.filename}
...
IMPORTANT: Respond ONLY with the complete markdown document. Start with YAML front matter (---). Do NOT acknowledge - respond immediately with the full document.""")
                    ]
                    
                    # Add validation feedback if retry
                    if validation_feedback:
                        gen_msg.append(HumanMessage(content=f"""
WARNING: VALIDATION ERRORS FROM PREVIOUS ATTEMPT:
{validation_feedback}

CORRECTION GUIDANCE:
- Review the EXEMPLAR above for exact format
- Check that section titles match exactly (e.g., "## 3. Primary Audience" not "Primary Architectural Components")
- Verify YAML frontmatter follows hard rules #1, #3, #4
- Ensure all required sections present: In Scope, Out of Scope, Primary Audience, Implementation Evidence

Fix these issues and regenerate the COMPLETE document."""))
                    
                    # API Call 2: Generation request with logging
                    api_start = time.time()
                    logger.info(f"API Call #{self.api_call_count + 1} - Generate {spec.filename} (attempt {attempt}/{max_retries})")
                    response = self.llm.invoke(gen_msg)
                    api_duration = time.time() - api_start
                    self.api_call_count += 1
                    self.total_api_time += api_duration
                    logger.info(f"API Call #{self.api_call_count} completed in {api_duration:.2f}s - Generated {len(response.content)} chars")
                    content = response.content
                    console.print(f"   [dim]  ✓ Document received (~{len(content)} chars)[/dim]")
            ...
            if validation.passed:
                ...
                if not self.dry_run:
                    ...
                    if spec.filename not in self.state["completed_files"]:
                ...
                # Log validation failure with content snippet for debugging
                content_snippet = content[:100].replace('\n', ' ') if content else "(empty)"
                console.print(f"   [dim]Content preview: {content_snippet}...[/dim]")
                
                if attempt < max_retries:
                    console.print(f"   [yellow]Retrying with validation feedback...[/yellow]")
                    # Build enhanced validation feedback mapping errors to rules
                    feedback_lines = []
                    for issue in validation.issues:
                        feedback_lines.append(f"ERROR: {issue}")
                        # Map common errors to hard rules
                        if "Primary Audience" in issue or "Primary Users" in issue:
                            feedback_lines.append("   -> Check exemplar section 3: Should be '## 3. Primary Audience'")
                        elif "YAML" in issue.lower() or "frontmatter" in issue.lower():
                            feedback_lines.append("   -> See hard rules #1, #3, #4 for YAML format")
                        elif "traceability" in issue.lower():
                            feedback_lines.append("   -> See hard rule #4: source-materials/requirements-v0.2/FILE.md#ID")
                        elif "numbering" in issue.lower():
                            feedback_lines.append("   -> See hard rule #2: All sections must be numbered consecutively")
                    validation_feedback = "\n".join(feedback_lines)
    ...
    def _generate_dry_run_content(self, spec: FileSpec) -> str:
        ...
        doc_type = spec.filename.replace('.md', '')
        title = spec.filename.replace('.md', '').replace('-', ' ').title()
        
        base_content = f"""---
document_type: {doc_type}
phase: {spec.phase}
audience: [Enterprise Architects, Operators, Auditors]
traceability: [src-v02/requirements.md]
---
...
"""
    ...
    def validate_file(self, content: str, spec: FileSpec) -> ValidationResult:
        """Run 5-part validation"""
        issues = []
        warnings = []
        
        # Part 1: YAML front matter structure
        yaml_result = validate_yaml_frontmatter(content)
        if not yaml_result["valid"]:
            issues.append(f"YAML Structure: {yaml_result['error']}")
        
        # Part 2: YAML field values
        yaml_values = validate_yaml_values(content)
        if not yaml_values["valid"]:
            if "errors" in yaml_values:
                issues.extend([f"YAML Value: {e}" for e in yaml_values["errors"]])
            else:
                issues.append(f"YAML Values: {yaml_values.get('error', 'Unknown error')}")
        
        # Part 3: Required sections
        sections = validate_required_sections(content, spec.phase)
        if not sections["valid"]:
            if "errors" in sections:
                issues.extend([f"Section: {e}" for e in sections["errors"]])
            else:
                issues.append(f"Sections: {sections.get('error', 'Unknown error')}")
        
        # Part 4: AI-consumability
        consumability = validate_consumability(content)
        issues.extend(consumability["errors"])
        warnings.extend(consumability["warnings"])
        
        # Part 5: Requirement traceability (if needed)
        if spec.requires_traceability:
    ...
    def _print_summary(self):
        """Print generation summary with timing information"""
        ...
        table.add_row("End Time", end_time.strftime("%Y-%m-%d %H:%M:%S"))
        ...
        # API call statistics
        table.add_row("Total API Calls", str(self.api_call_count))
        avg_api_time = self.total_api_time / self.api_call_count if self.api_call_count > 0 else 0
        table.add_row("Total API Time", f"{self.total_api_time:.1f}s")
        table.add_row("Avg API Call Time", f"{avg_api_time:.2f}s")
        table.add_row("", "")  # Separator
...
def main():
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Clean state and output directory before starting (fresh run)"
    )
    ...
    # From src/ -> 01-documentation-generator/ -> projects/ -> eva-foundation/ -> docs/ -> workspace_root
    workspace_root = script_dir.parent.parent.parent.parent.parent
    
    # Handle --clean flag
    if args.clean:
        eva_dir = workspace_root / "docs" / "eva-foundation"
        state_file = eva_dir / "generated-output" / "generation-state.json"
        output_dir = eva_dir / "generated-output" / "markdown"
        evidence_dir = eva_dir / "generated-output" / "evidence"
        
        console.print("[yellow]🧹 Cleaning state and output directories...[/yellow]")
        ...
        if evidence_dir.exists():
            ...
            console.print(f"   ✓ Removed evidence directory: {evidence_dir}")
        console.print("[green]✅ Clean complete - starting fresh generation[/green]\n")
